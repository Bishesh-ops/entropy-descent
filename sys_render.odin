...
sys_render_map :: proc(renderer: ^sdl.Renderer, game_map: ^Map, cam: Camera) {
	for x in 0 ..< MAP_WIDTH {
		for y in 0 ..< MAP_HEIGHT {
			...
			factor := f32(1.0) if tile.visible else f32(0.20)

			r := u8(f32(base_r) * factor)
			g := u8(f32(base_g) * factor)
			b := u8(f32(base_b) * factor)

			sdl.SetRenderDrawColor(renderer, r, g, b, 255)
			sdl.RenderFillRect(renderer, &rect)

			grid_alpha := u8(18) if tile.visible else u8(6)
...
sys_render_entities :: proc(renderer: ^sdl.Renderer, world: ^World, cam: Camera, gs: ^Game_State) {
	for &entity in world.entities {
		...
		if .Player not_in entity.components {
			ex := entity.position.x
			ey := entity.position.y
			if !gs.game_map.tiles[ex][ey].visible do continue
		}
