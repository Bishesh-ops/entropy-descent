...
PlayState::PlayState(Game &gameRef)
    : game(gameRef), gameMap(MAP_WIDTH, MAP_HEIGHT, 20), cameraX(0), cameraY(0),
      ...
      spatialGrid(MAP_WIDTH * MAP_HEIGHT,
  ...
  lua.set_function("SpawnProjectile",
                   [&](float x, float y, float dx, float dy, float speed,
                       int damage, float lifeTime, const std::string &element,
                       int r, int g, int b) {
                     ...
                     registry.emplace<RenderColor>(
                         proj, static_cast<uint8_t>(r), static_cast<uint8_t>(g),
                         static_cast<uint8_t>(b), static_cast<uint8_t>(255));
                     registry.emplace<Projectile>(proj, lifeTime, damage,
                                                  element, playerEntity);
                   });

  lua.set_function(
      "SpawnParticles", [&](float x, float y, int count, int r, int g, int b) {
        ...
        for (int i = 0; i < count; ++i) {
          ...
          registry.emplace<Particle>(
              p, life, life, std::cos(angle) * speed, std::sin(angle) * speed,
              static_cast<uint8_t>(r), static_cast<uint8_t>(g),
              static_cast<uint8_t>(b));
        }
      });

  lua.set_function("ConsumeTiles", [&](float pixelX, float pixelY,
                                       int gridRadius,
    ...
    if (tileType == "FIRE") {
      target = TileState::FIRE;
      aftermath = TileState::SCORCHED;
    }
    if (tileType == "WATER") {
      target = TileState::WATER;
      aftermath = TileState::FROZEN;
    }
  ...
  playerEntity =
      WorldBuilder::generateFloor(registry, gameMap, spatialGrid, MAP_WIDTH,
                                  MAP_HEIGHT, loadedEnemies, loadedItems);
}
  ...
  while (SDL_PollEvent(&event)) {
    ...
    if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
      if (event.button.button == SDL_BUTTON_LEFT) {
        ...
        dispatcher.trigger(
            SpellCastEvent{playerEntity, SpellCastEvent::SpellType::MELEE,
                           mouseX + cameraX, mouseY + cameraY, nullptr});
      }
    }
    ...
    if (event.type == SDL_EVENT_KEY_DOWN) {
      ...
      if (event.key.key == SDLK_E) {
        ...
        SDL_GetMouseState(&mouseX, &mouseY);
        dispatcher.trigger(
            SpellCastEvent{playerEntity, SpellCastEvent::SpellType::CRYO,
                           mouseX + cameraX, mouseY + cameraY, nullptr});
      }
      if (event.key.key == SDLK_F) {
        ...
        SDL_GetMouseState(&mouseX, &mouseY);
        dispatcher.trigger(
            SpellCastEvent{playerEntity, SpellCastEvent::SpellType::FIRE,
                           mouseX + cameraX, mouseY + cameraY, nullptr});
      }
...
void PlayState::update(float dt) {
  ...
  if (registry.all_of<Transform>(playerEntity)) {
    ...
    cameraX =
        std::clamp(static_cast<int>(pTransform.x) - (game.getWindowWidth() / 2),
                   0, (MAP_WIDTH * 20) - game.getWindowWidth());
    cameraY = std::clamp(static_cast<int>(pTransform.y) -
                             (game.getWindowHeight() / 2),
                         0, (MAP_HEIGHT * 20) - game.getWindowHeight());
  }
    ...
    if (eStats.entropy >= 86) {
      ...
      if (chanceDist(rng) < 10) {
        ...
        if (gameMap.isFloor(pX, pY)) {
          ...
          registry.emplace<Transform>(phantom, static_cast<float>(pX * 20),
                                      static_cast<float>(pY * 20));
          ...
          registry.emplace<RenderColor>(
              phantom, static_cast<uint8_t>(200), static_cast<uint8_t>(0),
              static_cast<uint8_t>(200), static_cast<uint8_t>(150));
        }
      }
    }
    cascadeTimer = 0.0f;
  ...
  if (eStats.entropy >= 100) {
    game.getStateMachine().pushState(
        std::make_unique<VowState>(game, registry, playerEntity));
  }
...
void PlayState::render() {
  renderSystem.update(game.getRenderer(), registry, gameMap, cameraX, cameraY,
                      game.getWindowWidth(), game.getWindowHeight(), uiRenderer,
                      playerEntity, floorDepth);
  ...
  for (auto entity : registry.storage<entt::entity>()) {
    ...
    if (registry.all_of<Player>(entity))
      ...
      nodeName += " [Item]";
    else if (registry.all_of<Projectile>(entity))
      nodeName += " [Projectile]";
    ...
    if (ImGui::TreeNode((void *)(intptr_t)eId, "%s", nodeName.c_str())) {
      ...
      if (registry.all_of<Health>(entity)) {
        ...
        ImGui::ProgressBar(
            static_cast<float>(hp.current) / hp.max, ImVec2(-1.0f, 0.0f),
            (std::to_string(hp.current) + "/" + std::to_string(hp.max))
                .c_str());
      }
...
void PlayState::updateSpatialGrid(entt::entity entity, int oldX, int oldY,
                                  int newX, int newY) {
...
void PlayState::onDescend(const DescendEvent &event) {
  ...
  floorDepth++;
  if (floorDepth > 10) {
    game.getStateMachine().pushState(std::make_unique<WinState>(game));
    return;
  }
  WorldBuilder::repopulateFloor(registry, gameMap, spatialGrid, MAP_WIDTH,
                                MAP_HEIGHT, loadedEnemies, loadedItems,
                                playerEntity, floorDepth);
  needsFOVUpdate = true;
}
