...
void UIRenderer::render(SDL_Renderer *renderer, const entt::registry &registry,
                        entt::entity playerEntity, int windowWidth,
                        int windowHeight, int floorDepth) {
  ...
  float hpPercent = std::clamp(
      static_cast<float>(hp.current) / static_cast<float>(hp.max), 0.0f, 1.0f);
  if (hpPercent > 0.0f) {
  ...
  if (registry.all_of<Inventory>(playerEntity)) {
    ...
    for (int i = 0; i < inv.maxCapacity; ++i) {
      SDL_FRect slotRect = {barX + i * (slotSize + padding), slotY, slotSize,
                            slotSize};
  ...
  for (int i = 0; i < displayFloors; ++i) {
    SDL_FRect square = {startX + (i * 10.0f), startY, 8.0f, 8.0f};
