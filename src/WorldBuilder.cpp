...
entt::entity WorldBuilder::generateFloor(entt::registry &registry, Map &gameMap,
                                         std::vector<entt::entity> &spatialGrid,
                                         int mapWidth, int mapHeight,
                                         const std::vector<EnemyDef> &enemies,
                                         const std::vector<ItemDef> &items) {
  std::fill(spatialGrid.begin(), spatialGrid.end(),
  ...
  for (int i = 0; i < 40; ++i) {
    ...
    for (int dy = -2; dy <= 2; ++dy) {
      for (int dx = -2; dx <= 2; ++dx) {
        if (gameMap.isFloor(cx + dx, cy + dy) &&
            chance(rng) < (100 - ((std::abs(dx) + std::abs(dy)) * 25)))
  ...
  registry.emplace<Transform>(playerEntity, static_cast<float>(startX * 20),
                              static_cast<float>(startY * 20));
  ...
  if (!enemies.empty()) {
    ...
    for (int i = 0; i < 60; ++i) {
      ...
      while (!gameMap.isFloor(ex, ey) ||
             ...
             spatialGrid[ey * mapWidth + ex] != entt::null) {
        ex = distX(rng);
        ey = distY(rng);
      }
      ...
      registry.emplace<Transform>(enemy, static_cast<float>(ex * 20),
                                  static_cast<float>(ey * 20));
  ...
  if (!items.empty()) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      registry.emplace<Transform>(item, static_cast<float>(ix * 20),
                                  static_cast<float>(iy * 20));
  ...
  while (true) {
    int sx = distX(rng);
    int sy = distY(rng);
    if (gameMap.isFloor(sx, sy) &&
        spatialGrid[sy * mapWidth + sx] == entt::null) {
      ...
      if (dist >= 30) {
        ...
        registry.emplace<RenderColor>(
            stairs, static_cast<uint8_t>(255), static_cast<uint8_t>(255),
            static_cast<uint8_t>(0), static_cast<uint8_t>(255));
        registry.emplace<Transform>(stairs, static_cast<float>(sx * 20),
                                    static_cast<float>(sy * 20));
...
void WorldBuilder::repopulateFloor(entt::registry &registry, Map &gameMap,
                                   std::vector<entt::entity> &spatialGrid,
                                   int mapWidth, int mapHeight,
                                   const std::vector<EnemyDef> &enemies,
                                   const std::vector<ItemDef> &items,
                                   entt::entity existingPlayer,
                                   int floorDepth) {
  std::fill(spatialGrid.begin(), spatialGrid.end(),
            static_cast<entt::entity>(entt::null));
  ...
  registry.emplace_or_replace<Transform>(existingPlayer,
  ...
  if (!enemies.empty()) {
    ...
    for (int i = 0; i < 60; ++i) {
      ...
      const auto &def = enemies[distArch(rng)];
      auto enemy = registry.create();
      ...
      registry.emplace<Transform>(enemy, static_cast<float>(ex * 20),
                                  static_cast<float>(ey * 20));
      registry.emplace<Velocity>(enemy, 0.0f, 0.0f, 75.0f);
      registry.emplace<RenderColor>(enemy, def.r, def.g, def.b,
      ...
      registry.emplace<Health>(
          enemy, std::max(1, static_cast<int>(def.hp * scaleMultiplier)));
      registry.emplace<CombatStats>(
          enemy, std::max(1, static_cast<int>(def.attack * scaleMultiplier)),
  ...
  if (!items.empty()) {
    ...
    for (int i = 0; i < 20; ++i) {
      int ix = distX(rng), iy = distY(rng);
      while (!gameMap.isFloor(ix, iy) ||
             spatialGrid[iy * mapWidth + ix] != entt::null) {
        ix = distX(rng);
        iy = distY(rng);
      }

      const auto &def = items[distItem(rng)];
      auto item = registry.create();
      registry.emplace<Position>(item, ix, iy);
      registry.emplace<RenderColor>(item, def.r, def.g, def.b,
                                    static_cast<uint8_t>(255));
      registry.emplace<Item>(item);
      registry.emplace<ItemEffect>(item, def.effectType, def.magnitude);
      registry.emplace<Transform>(item, static_cast<float>(ix * 20),
